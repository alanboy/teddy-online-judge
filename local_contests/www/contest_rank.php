...
function start(){
	...
	<table border='0' cellspacing="5" style="font-size: 14px; padding: 5px" > 
	<thead>
		<tr align=center>
		<th ></th>
		<?php
		...
		<div class="post" >
			...
			function showRuns(){
				...
				$("#runs_div").fadeOut("fast", function (){
					...
					{	
						...
						html +=  "<TD align='center' >" + (parseInt(CurrentRuns[a].tiempo)/1000).toFixed(3) +" S </TD>";
			...
			function runsCallback(data){
				...
				if(CurrentRuns.length == data.length){
					...
					// buscar si algunos runs tiene 
					// diferente contestacion
					for (var cri = CurrentRuns.length - 1; cri >= 0; cri--) {
						for (var i = data.length - 1; i >= 0; i--) {
							if(	
								(data[i].execID == CurrentRuns[cri].execID)							
							){
								//console.log(data[i].execID );
								//console.log(data[i].status, CurrentRuns[cri].status)	

								if( CurrentRuns[cri].status.indexOf( data[i].status, 0) == -1){
									//console.warn(data[i].status, CurrentRuns[cri].status)
									CurrentRuns = data;
									showRuns();
									return;
								}
